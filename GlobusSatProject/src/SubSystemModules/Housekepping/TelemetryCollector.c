...
void TelemetryCollectorLogic()
{
	if (CheckExecutionTime){
		...
		//TelemetrySaveTRXVU();
		TelemetrySaveWOD();
		vTaskDelay(1000); // wait one sec (~1000 ticks) remove this as this is just for testing so we don't save all the time
	}
}
 ...
 *  Struct for testing.
 */
typedef struct __attribute__ ((__packed__)) testWOD_t
{
	size_t heapSize;
}TestWOD;
...
void TelemetrySaveWOD()
{
	...
	if (err!=-1){
		...
		c_fileWrite(FILENAME_WOD_TLM, &wodData);
	}
}
